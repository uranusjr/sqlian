...
# built documents.
#
with open(os.path.join(PROJECT_ROOT, 'sqlian', '__init__.py')) as f:
    for line in f:
        if line.startswith('VERSION'):
            parts = [str(v) for v in eval(line[len('VERSION = '):])]
            break
# The short X.Y version.
version = '.'.join(parts[:2] if parts[2] == '0' else parts[:3])
